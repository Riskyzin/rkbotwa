...
async function startClient() {
    const client = clientConnect({
        ...
        browser: ['Client Multi Device','Safari','1.0.0'],
        auth: state
    })
    ...
    client.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./roother")(client, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
    ...
    client.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                    client.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `Seja bem-vindo ao grupo ${metadata.subject}, @${num.split("@")[0]}!` })
                } else if (anu.action == 'remove') {
                    client.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `@${num.split("@")[0]} Saiu do grupo ${metadata.subject}` })
                } 
            }
        } catch (err) {
            console.log(err)
     ...
     * @returns 
     */
         client.sendImageAsStickernoreply = async (jid, path, quoted, options = {}) => {
            let buff = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
            let buffer
            if (options && (options.packname || options.author)) {
                buffer = await writeExifImg(buff, options)
